    ...
    <li class="dd-item dd3-item" data-id="{{$item["id"]}}">
        ...
        <div class="dd3-content {{$item["url"] == 'javascript:;' ? "font-weight-bold" : ""}}">
            ...
            <form action="{{route("menu.eliminar", $item["id"])}}" class="form-eliminar-menu d-inline" method="POST">
                @csrf @method('delete')
                <button type="button" class="btn-accion-tabla float-right boton-eliminar-menu" data-toggle="tooltip" title="Eliminar menu"><span class="text-danger fas fa-trash"></span></button>
            </form>
        </div>
    </li>
@else
